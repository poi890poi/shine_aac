...
try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  let changedAlpha=0,restoredAlpha=0,preservedWhite=0,poses=0,nativeErrors=0,maskErrors=0;const birds=[];
  for(const [id,bird] of Object.entries(manifest.birds)){
   ...
   for(let p=0;p<bird.poses.length;p++){
    ...
    const protection=rule.birds[id]?.foregroundMasks?.[p];const protectedPixels=protection?raster(await image('rules/'+protection.file)):null;
    const expected=seeds.reduce((sum,s)=>sum+s.expectedArea,0);let actual=0,restored=0;
    for(let i=0;i<w*h;i++){
     const alpha=mask[i*4];if(alpha===255&&old[i*4+3]===0){restored++;if(!protectedPixels||protectedPixels[i*4]!==255)maskErrors++;}
     else if(alpha!==old[i*4+3]){actual++;if(alpha!==0||!seeds.some(s=>{const [x,y,bw,bh]=s.bounds;return i%w>=x&&i%w<x+bw&&Math.floor(i/w)>=y&&Math.floor(i/w)<y+bh;}))maskErrors++;}
     if(alpha===255&&raw[i*4]>220&&raw[i*4+1]>220&&raw[i*4+2]>220)white++;
    }
    if(actual!==expected||restored!==(protection?.opaquePixels??0))maskErrors++;restoredAlpha+=restored;for(const s of seeds)if(mask[(s.point[1]*w+s.point[0])*4]!==0)maskErrors++;
  ...
  return {poses,changedAlpha,restoredAlpha,preservedWhite,maskErrors,nativeErrors,birds,cloudDifferences,birdNativeDraws:draws,rescaled};
 });
