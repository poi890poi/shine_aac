...
devices={}
for device,pattern in (('phone','phone-foreground'),('tablet','tablet-foreground'),('tablet-awake','tablet-foreground-awake')):
    runs=[]
    for directory in sorted(base.glob(pattern+'-[0-3]-*')):
        ...
        value['endingInteractive']=raw.get('endingInteractive')
        value['endingControlWakeLockHeld']=raw.get('endingControlWakeLockHeld')
    ...
    if len({r['controlKeepAwake'] for r in runs})!=1: reasons.append('Wake-lock method differs within the batch')
    ...
    devices[device]={'runs':runs,'pairedComparisonUsable':not reasons,'limitations':reasons,
                     ...
                     'backgroundForegroundMeanP95Ratio':statistics.mean(b)/statistics.mean(a) if a and b and not reasons else None}
