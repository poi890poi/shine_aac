...
await copyDirectoryInPlace(join(moduleRoot,'assets','native'),join(output,'assets','native'));
await copyDirectoryInPlace(join(moduleRoot,'assets','candidates','second-bird-20260906'),join(output,'assets','candidates','second-bird-20260906'));
for(const entry of await readdir(join(moduleRoot,'assets','candidates','all-birds-20260906'),{withFileTypes:true})){
  if(entry.isFile()&&entry.name.endsWith('.png'))await copyInPlace(join(moduleRoot,'assets','candidates','all-birds-20260906',entry.name),join(output,'assets','candidates','all-birds-20260906',entry.name));
}
